...
export function EntidadeTimeline({
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const j = (await res.json().catch(() => ({}))) as { data?: Registro[]; is_owner?: boolean; error?: string };
  ...
  async function registrarNota() {
    ...
    try {
      ...
      if (!res.ok || !j.ok) { setErro(j.error || "Não foi possível registrar a nota."); return; }
      setNota("");
      await carregar();
    } catch {
  ...
  async function salvarEdicao(id: string) {
    ...
    else { const j = (await res.json().catch(() => ({}))) as { error?: string }; setErro(j.error || "Não foi possível editar."); }
  }
  ...
  async function arquivar(id: string) {
    ...
    else { const j = (await res.json().catch(() => ({}))) as { error?: string }; setErro(j.error || "Não foi possível arquivar."); }
  }
  ...
  async function abrirLogs() {
    ...
    try {
      ...
      const j = (await res.json().catch(() => ({}))) as { data?: LinhaLog[]; error?: string };
  ...
  return (
    <section className="rounded-xl border border-obra-borda bg-obra-dark-2 p-4">
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="ml-auto flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                      {r.pode_editar && !editando && (
                        <button type="button" onClick={() => { setEditandoId(r.id); setEditTexto(r.descricao ?? ""); }} title="Editar" className="text-obra-texto-3 hover:text-obra-dourado">
                          <Pencil size={13} />
                        </button>
                      )}
                      {r.pode_arquivar && !editando && (
                        <button type="button" onClick={() => void arquivar(r.id)} title="Arquivar" className="text-obra-texto-3 hover:text-[#ff7b72]">
                          <Archive size={13} />
                        </button>
                      )}
                    </span>
                  ...
                  {editando ? (
                    <div className="mt-1">
                      ...
                      <div className="mt-1 flex gap-1.5">
                        <button type="button" onClick={() => void salvarEdicao(r.id)} className="inline-flex items-center gap-1 rounded-md bg-obra-dourado/20 px-2 py-0.5 text-[11px] font-semibold text-obra-dourado">
                          <Check size={12} /> Salvar
                        </button>
                        <button type="button" onClick={() => { setEditandoId(null); setEditTexto(""); }} className="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[11px] text-obra-texto-3">
                          <X size={12} /> Cancelar
                        </button>
                      </div>
                    </div>
      ...
      {logsOpen && (
        <div role="dialog" aria-modal="true" onClick={() => setLogsOpen(false)} className="fixed inset-0 z-[240] flex items-start justify-center overflow-y-auto bg-black/70 p-6">
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-2xl rounded-2xl border border-obra-borda bg-obra-dark-2 p-5">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h2 className="m-0 flex-1 text-base font-semibold text-obra-texto">Logs da IA — relatório</h2>
