...
export const COPILOTO_FERRAMENTAS_ESCRITA_FASE3: HubAgenteFerramentaId[] = [
  ...
  "hub_obra_sc_criar",
  // E6 — preparar pagamento (opera sobre obra_id, não sobre lead). PREPARA + enfileira a dupla
  // aprovação; LIBERAR o escrow é decisão humana com as 2 chaves na tela (NUNCA por voz).
...
- hub_obra_financeiro_resumo (LEITURA): resumo financeiro da obra (orçado, aprovado, a pagar, vencendo, atrasado, em custódia, aguardando 2ª chave, cobertura). Use para "como está o financeiro da obra", "quanto falta aprovar", "o que está atrasado para pagar", "quanto tem em custódia". Params: { "obra_id": "<id da tela>" }.
- hub_obra_pagamento_preparar (ESCRITA): prepara um pagamento/medição e o enfileira para a APROVAÇÃO DUPLA (arquitetura + Hub). Use para "lança a medição 3 de R$84 mil da elétrica", "prepara o pagamento da fundação". Params: { "obra_id": "<id>", "titulo": "<ex.: Medição 3 — Elétrica>", "valor": <número>, "data_vencimento": "AAAA-MM-DD", "tipo"?: "medicao|adiantamento|retencao|aditivo|reembolso|avulso", "orcamento_id"?: "<id>", "adiantamento_justificativa"?: "<obrigatório se tipo=adiantamento>" }. O pagamento NÃO é aprovado nem o escrow liberado por aqui — LIBERAR O DINHEIRO é decisão HUMANA com as duas chaves na tela (NUNCA por voz). Se o orçamento da frente não estiver aprovado, o pagamento nasce BLOQUEADO (o sistema avisa). Em descricao_humana: "Vou preparar o pagamento <titulo> (R$<valor>) e enviar para aprovação dupla".`;
