...
--
-- VERSIONAMENTO da função já aplicada em produção em 2026-06-24 via execute_sql
-- (ver docs/sql/codigos-rastreio-APPLIED.sql). Sem este arquivo, ambientes novos
-- (dev/staging/reset) não têm a rpc e caem no fallback COUNT(*)+1 do app — que
-- tem corrida (2 cadastros simultâneos podem gerar o mesmo código).
--
-- IDEMPOTENTE: CREATE TABLE IF NOT EXISTS + CREATE OR REPLACE FUNCTION. Pode ser
-- reaplicado com segurança. JÁ ESTÁ EM PRODUÇÃO — não reaplicar lá; este arquivo
-- só garante paridade nos demais ambientes.
--
