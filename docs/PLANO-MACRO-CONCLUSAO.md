...
- **💳 Tijolos — metering de IA (Fases 1, 2 e 4 ENTREGUES + migração aplicada):**
  - **Fase 1 (motor):** moeda "Tijolos" (R$0,10, markup 10×, pré-pago, Claude=Turbo); `lib/ia/metering-calc.ts` + `lib/ia/metering.ts` + medição sombra no motor + `GET /api/crm/ia/creditos`; 4 tabelas `hub_ia_*` (RLS, advisors limpos).
  - **Fase 2 (carteira):** `/crm/creditos` — saldo + extrato em Tijolos (nunca R$), no menu IA (gestor+).
  - **Fase 4 (super-admin):** `/crm/precificacao` (owner) + rotas `GET/PUT /api/crm/ia/config` e `/precos` — edita moeda/valor/markup/câmbio/modo + preços por modelo + **visão de margem** (R$). Tudo verificado clicando.
  - Spec `docs/superpowers/specs/2026-06-26-creditos-ia-metering-design.md` · plano `docs/superpowers/plans/2026-06-26-creditos-ia-fase1.md`. **Resta:** Fase 3 (hard-cap + comprar Tijolos — TRAVA: gateway de pagamento) + painel de previsão.
